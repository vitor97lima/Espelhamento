...
{
    Mat frame = imread("/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/img1.jpg", COLOR_BGR2RGB);
    Mat rosto;
    Mat eye;
    Mat mouth;
    ...
    //{
        //Mat frame;
        //camera >> frame;
        rosto = faceDetector.getFace(frame);

        eye = eyesDetector.getEyes(rosto);
        mouth = mouthDetector.getMouth(rosto);
        if(!(frame.empty()))
            imshow("frame", frame);
        if(!(rosto.empty()))
            imshow("rosto", rosto);
...
int main(){
    ...
    String images_path = "/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/lfpw/images_train.txt";
    String annotations_path = "/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/lfpw/annotation_train.txt";
    vector<String> images_train;
    vector<String> landsmark_train;
    loadDatasetList(images_path, annotations_path, images_train, landsmark_train);

    Mat image;
    vector<Point2f> facial_points;
    for(size_t i=0 ; i< images_train.size() ; i++){
        image = imread(images_train[i].c_str());
        loadFacePoints(landsmark_train[i], facial_points);
    ...
    */
    ...
    String test_images_path = "/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/lfpw/images_test.txt";
    String testFiles;
    String testPts;
    ...
    vector<Rect> faces;
    CascadeClassifier face_cascade("/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/haarcascades/haarcascade_frontalface_alt.xml");
    CascadeClassifier eyes_cascade("/home/vitor/Desenvolvimento/Qt Projects/Espelhamento/haarcascades/haarcascade_eye.xml");
...
bool myDetector(InputArray image, OutputArray faces, CascadeClassifier *face_cascade)
{
    ...
    //face_cascade->detectMultiScale(gray, faces_, 1.1, 2, 0|CASCADE_SCALE_IMAGE, Size(60, 60));
